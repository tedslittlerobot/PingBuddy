// Taken from https://github.com/samiyr/SwiftyPing
...
public class SwiftyPing: NSObject {
    ...
    private func sendPing() {
        ...
        currentQueue.async {
            ...
            guard let icmpPackage = self.createICMPPackage(
                identifier: UInt16(self.identifier),
                sequenceNumber: UInt16(self.sequenceIndex),
                payloadSize: Int(self.configuration.payloadSize)
    ...
    private func computeCheckSum(buffer: UnsafeMutableRawPointer, bufLen: Int) -> UInt16 {
        ...
        // @HACK - edited to add max to ensure int is positive or zero
